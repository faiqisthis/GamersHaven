...
function Explore() {
  ...
  const filteredProducts = consoles.filter((item) => {
    // Category filter
    ...
    // // Rating filter
    // if (rating === "4+" && item.rating < 4) return false;
    // if (rating === "3+" && item.rating < 3) return false;
    // if (rating === "2+" && item.rating < 2) return false;
          ...
          <div className="grid xl:grid-cols-3 md:grid-cols-2 gap-6 flex-1 md:col-span-3">
            ...
            {filteredProducts.length === 0 ? (
              ...
              filteredProducts.map((item) => (
                <Card key={item._id || item.id} console={item} />
              ))
            )}
          </div>
        </div>
